...
import { RenderResult, fireEvent, /* prettyDOM,  */render } from "@testing-library/react";
...
describe("Tests for Blog component", () => {
  let blog: IBlog, user: loginResponse, setBlogs: jest.Mock, setNotification: jest.Mock, blogComponent: RenderResult;
  ...
  beforeEach(() => {
    ...
    blogComponent = render(
      <Blog id="test-blog" blog={blog} setBlogs={setBlogs} user={user} setNotification={setNotification} />
    );
  });

  test("Blog component must show title and author's blog, but don't show URL and likes counting when toggled off", () => {
    const visibleComponent = blogComponent.container.querySelector(".togglable-container :not([style*='display:none'])")!;

    expect(visibleComponent).toHaveTextContent(
      `${blog.title} by ${blog.author || ""}`
    );
    expect(visibleComponent).not.toHaveTextContent(
      `${blog.url}`
    );
    expect(visibleComponent).not.toHaveTextContent(
      `${blog.likes || 0}`
